...
ParZu misses the lemma for a wider set of words: it strips the `-e` off weak masculines and lands on
a different real word (`Zeuge` → `Zeug`, `Bürge` → `Burg`, `Schütze` → `Schütz`) or appends one
(`Handlanger` → `Handlange`). The list lookup then finds nothing — `Der Zeuge sagte aus.` was not
markable at all — or the wrong entry. `mark_nouns` therefore prefers the **surface form** over the
lemma whenever the form is in `Lexicon.WORD_LIST_ENTRIES` and the lemma is not; where both are
entries (`Ahne`/`Ahn`, `Patrone`/`Patron`) ParZu's lemma stands. A sweep over all 3926 entries found
31 such words, so the rule replaces a hand-kept table.
