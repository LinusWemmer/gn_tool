...
class Lexicon_Fem:
    ...
    def feminize_article(word_parse) -> str:
        ...
        if feats[0] == "Def":
            if feats[2] == "_":
                feats[2] = "Nom"
            article =  Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_DER, feats[2])
            return article.capitalize() if word_parse[1][0].isupper() else article
        # Case Indifinitive Artikels, only ein
        elif feats[0] == "Indef":
            article = "ein" +  Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_EIN, feats[2])
            return article.capitalize() if word_parse[1][0].isupper() else article
        else:
            ...
            for start in Lexicon_Fem.JEDER_PARADIGM:
                if word.startswith(start):
                    ...
                    if feats[1] == "_":
                        feats[1] = "Nom"
                    article = start + Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_JEDER, feats[1])
                    return article.capitalize() if word_parse[1][0].isupper() else article
            # Ein-Paradigm: einer, keiner, meiner, deiner, seiner, ihrer, enser 
            for start in Lexicon_Fem.EIN_PARADIGM:
                if word.startswith(start):
                    article = start + Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_EIN, feats[1])
                    return article.capitalize() if word_parse[1][0].isupper() else article
            if re.match(r"(U|u)(nser|nsre|nsere)", word):
                article = Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_UNSER, feats[1])
                return article.capitalize() if word_parse[1][0].isupper() else article
            elif re.match(r"(E|e)(uer|ure)", word):
                article = Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_EUER, feats[1])
                return article.capitalize() if word_parse[1][0].isupper() else article
    ...
    def feminize_adjectives(word_parse, has_article) -> str:
        ...
        if feats[2] == "_":
            feats[2] = "Nom"
        adjective =  adjective + Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_JEDER, feats[2])
        return adjective.capitalize() if word_parse[1][0].isupper() else adjective
    ...
    def feminize_pronoun(word_parse,has_article) -> str:
        ...
        if word_parse[4] == "PPER":
            ...
            if feats[0] == "3":
                pronoun = Lexicon_Fem.case_ending(Lexicon_Fem.PRONOUNS, feats[3])
            ...
            if word_parse[1].endswith("as"):
                ...
                if has_article:
                    ...
                    pronoun = word_parse[2][:-1] + Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_JEDER, feats[1])
            return pronoun.capitalize() if is_capitalized else pronoun
        elif word_parse[4] == "PRELS" and word_parse[1].startswith("d"):
            if feats[1] == "_":
                feats[1] = "Nom"
            pronoun = Lexicon_Fem.case_ending(Lexicon_Fem.ARTIKEL_DER, feats[1])
            return pronoun.capitalize() if is_capitalized else pronoun
